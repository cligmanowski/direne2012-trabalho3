...
% Ideias para a FAE
% 1 - Contar o número de peças no tabuleiro
% 2 - Número de peças no pote for >= 12, jogo vencido/perdido
% 3 - Número de casas com uma peça
% 4 - Número de casas com nenhuma peça
% Aqui A é maquina
fae([PA,_,_,_],_,F):-
    eh_maquina(a),
    PA >= 12,
...
fae([PA,TA,TB,PB],b,F):-        %A jogou
    eh_maquina(a),
    soma(TA,TPA),               %Pontuação das peças no tabuleiro
    soma(TB,TPB),
    casas_com_um(TA,NUTA),
    casas_com_um(TB,NUTB),
    casas_com_zero(TA,NZTA),
    casas_com_zero(TB,NZTB),
    F is (TPA - TPB) + (NUTA*(-2) + NUTB*2) + (PA*4 - PB*4) +
(NZTA*(-2) + NZTB*2),!.
...
fae([PA,TA,TB,PB],a,F):-        %B jogou
    ...
    casas_com_zero(TA,NZTA),
    casas_com_zero(TB,NZTB),
    F is (-1)*(TPB - TPA) + (NUTA*(-2) + NUTB*2) + (-1)*(PB*4 - PA*4)
+ (NZTA*(-2) + NZTB*2),!.
...
fae([PA,TA,TB,PB],a,F):- %B jogou
    ...
    casas_com_zero(TA,NZTA),
    casas_com_zero(TB,NZTB), 
    F is (TPB - TPA) + (NUTA*2 + NUTB*(-2)) + (PB*4 - PA*4) + (NZTA*2 +
NZTB*(-2)),!.
...
fae([PA,TA,TB,PB],b,F):- %A jogou
    ...
    casas_com_zero(TA,NZTA),
    casas_com_zero(TB,NZTB), 
    F is (-1)*(TPA - TPB) + (NUTA*2 + NUTB*(-2)) + (-1)*(PA*4 - PB*4) +
(NZTA*2 + NZTB*(-2)),!.
...
casas_com_zero([],0):-!.
casas_com_zero([0|X],V):-
        casas_com_zero(X,V2),
        V is V2 + 1,!.
casas_com_zero([_|X],V):-
        casas_com_zero(X,V2),
        V is V2,!.
